...
function loadGraphLinks() {
    fetch('/api/graphs_list').then(r => r.json()).then(j => {
        if (!j.success) return;
        const files = j.files || [];
        const genLink = document.getElementById('linkGeneralGraph');
        const hlLink = document.getElementById('linkHighlightGraph');
        if (files.includes('grafo_general.html')) {
            genLink.href = '/static/graphs/grafo_general.html'; genLink.style.display='inline-block';
        }
...
function mostrarTab(tabName, btnEl) {
    ...
    if (tabName === 'grafo') setTimeout(initCytoscape, 100);
}
...
function generateGeneralGraph() {
    ...
    .then(r => r.json())
    .then(j => {
        btn.disabled = false; btn.innerText = '🗺️ Generar y ver grafo general';
        if (j.success && j.path) {
            const link = document.getElementById('linkGeneralGraph');
            link.href = j.path;
            link.style.display = 'inline-block';
            window.open(j.path, '_blank');
        } else {
            alert('Error generando grafo: ' + (j.error || 'Desconocido'));
        }
    }).catch(e => { btn.disabled = false; btn.innerText = '🗺️ Generar y ver grafo general'; alert('Error: '+e.message); });
}
...
function initCytoscape() {
    ...
    fetch('/api/graph').then(r => {
        ...
        ], layout: { name: 'cose', directed: true, padding: 20 } });
